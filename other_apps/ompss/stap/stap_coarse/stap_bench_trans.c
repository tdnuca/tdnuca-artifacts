...
//arico: commenting this out, and moving it to init_tab_coh
//#pragma omp task input(iv, nv, dim, Tr) output(tab_coh)
...
#pragma omp task out(tab_coh[0;nv][0;dim]) label(init_tab_coh)
...
/************************************************************************/
//arico: commenting this out, and moving it to Calc_sterrvect
...
#pragma omp task out(Steervect[0;nth][0;nv][0;ntt*nsa]) label(Calc_steervect)
...
/************************************************************************/
#pragma omp task in(ptrfiltre[0;tf]) out(ptrout[0;nrg-tf+1]) label(Pulse_Comp)
void Pulse_Comp(int nrg, Cplfloat ptrin[nrg], int tf, 
...
/************************************************************************/
#pragma omp task in(In[0;nsa][0;nrec]) out(Out[0;ntt*nsa][0;ntt*nsa]) label(CovAvCov)
...
/************************************************************************/
#pragma omp task in(In[0;ntt*nsa][0;ntt*nsa]) out(Out[0;ntt*nsa][0;ntt*nsa]) label(Mat_Invert)
...
#pragma omp task in(CovInv[0;ntt*nsa][0;ntt*nsa], SteerVect[0;Nv][0;ntt*nsa]) out(Fil[0;Nv][0;ntt*nsa]) label(Calc_Filter)
...
#pragma omp task in(Sig[0;Nrec][0;Na], Fil[0;Nv][0;Ntt*Na]) out(Out[0;Nv][0;Nrec-Ntt*1]) label(Apply_Filter_task)
...
/************************************************************************/
#pragma omp task in(ptrin1[0;nv][0;ndop], ptrin2[0;nv][0;ndop]) out(ptrout[0;nv]) label(Int_Dop)
...
//arico: commenting this out, and movint it to average_power
//#pragma omp task input(Nth, Nrg, Nv, ptrin) output(Pow)
...
#pragma omp task in(ptrin[0;Nth][0;Nrg][0;Nv]) out(Pow[0;Nth]) label(average_power)
...
//arico: commenting this out, and moving it to tfac
//#pragma omp task input(rg, Nth, Nrg, Nv, In, tfac_L_V, tfac_L_RG, tfac_K, tfac_AVPWK) output(TOut)
...
#pragma omp task in(In[0;Nth][0;Nrg][0;Nv]) out(TOut[0;Nth][0;Nrg-tfac_L_RG+1][0;Nv-tfac_L_V+1]) label(tfac)
...
//arico: commenting this out, and moving it to tfac_add_edges
//#pragma omp task input(rg, Nrg, Nv, In, Pow, NrgLarge, NvLarge) output(TOut)
...
#pragma omp task in(In[0;Nth][0;Nrg][0;Nv], Pow[0;Nth]) out(TOut[0;Nth][0;NrgLarge][0;NvLarge]) label(tfac_add_edges)
...
#pragma omp task in(In[0;nv][0;Nth]) out(Out[0;nv][0;Nth]) label(MaxPower_task)
...
#pragma omp task in(In[0;Nth][0;Nv]) out(Out[0;Nth][0;Nv]) label(CorrecV)
...
/************************************************************************/
//arico: commenting this out, and moving it to X_1
//#pragma omp task input(dim2, dim3, a) output(b)
...
#pragma omp task in(a[0;dim1][0;dim2][0;dim3]) out(b[0;dim1][0;dim3][0;dim2]) label(X_1)
...
//arico: commenting this out, and moving it to X_3
//#pragma omp task input(dim3, a) output(b)
void X_3_1(int dim3, 
	   Cplfloat a[dim3],
	   Cplfloat b[dim3]) 
{
...
#pragma omp task in(a[0;dim1][0;dim2][0;dim3]) out(b[0;dim2][0;dim1][0;dim3]) label(X_3)
...
#pragma omp task in(a[0;dim1][0;dim2]) out(b[0;dim2][0;dim1]) label(X_5)
...
int main() {
     ...
     printf("%d\n", Nraf);
     for (j=0; j<5; j++) { // arico: repeat several times to make it long enough for benchmarking	//vdimic: removing this, too long for TaskSim tracing
     for (i=0; i<Nraf; i++) {
     ...
     } //vdimic: removing repetition for
#pragma omp taskwait
