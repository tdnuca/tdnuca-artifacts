...
#pragma omp task label(init_task)               \
  out(([num_vars*box_size]array)[start*box_size;number*box_size]) \
  firstprivate(array, seed, start, number)
              {
                for (int var = start; var < start+number; ++var)
                  init_task(x_block_size + 2,
                            y_block_size + 2,
                            z_block_size + 2,
                            &array[var*box_size],
                            seed + var);
   ...
   for (int start = 0; start < num_vars; start += comm_vars) {
     ...
     check_sum(start, number, grid_sum, 1);
   }
}
