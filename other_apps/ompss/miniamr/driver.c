...
void driver(void)
{
   int ts, start, number, stage, comm_stage;
   double t1, t2, t3, t4;
   double sum[num_vars];
   ...
   for (comm_stage = 0, ts = 1; ts <= num_tsteps; ts++) {
      for (stage = 0; stage < stages_per_ts; stage++, comm_stage++) {
         ...
         for (start = 0; start < num_vars; start += comm_vars) {
            ...
            int cs = checksum_freq && !(stage%checksum_freq);
            if (merge_checksum && cs)
               stencil_calc_checksum(start, number, sum);
            else {
               stencil_calc(start, number);
               if (cs)
                  check_sum(start, number, sum, 1);
            }
            t3 = timer();
            timer_cs_all = timer_calc_all = t3 - t4;
            if (checksum_freq && !(stage%checksum_freq)) {
#pragma omp task label(check_sum_val)                                   \
  inout(([num_vars]sum)[start;number])                                  \
  inout(([num_vars]grid_sum)[start;number])                             \
  firstprivate(start, number, report_diffusion, my_pe, ts, sum,         \
               grid_sum, tol)
              for (int var = start; var < (start+number); var++) {
                if (report_diffusion && !my_pe)
                  printf("%d var %d sum %lf old %lf diff %lf tol %lf\n",
                         ts, var, sum[var], grid_sum[var],
