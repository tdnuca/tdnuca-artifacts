         ...
         if (blocks[n].number >= 0 && blocks[n].level == level) {
            ...
            if (bp->refine == 1) {
               ...
               for (o = 0; o < 8; o++) {
                  ...
                  double * array_from = bp->array;
                  double * array_to = bp1->array;
...
#pragma omp task label(split_block)                                     \
  in(([num_vars*box_size]array_from)[start*box_size;number*box_size])   \
  out(([num_vars*box_size]array_to)[start*box_size;number*box_size])    \
  firstprivate(x_block_size, y_block_size, z_block_size,                \
               array_from, array_to, start, number)                     \
  firstprivate(i1, j1, k1, box_size)                                    \
  default(none)
                    {
                      for (int v = start; v < start+number; v++) {
                        split_block_task(x_block_size+2, y_block_size+2, z_block_size+2,
                                         &array_from[v*box_size],
                                         &array_to[v*box_size],
                                         i1, j1, k1);
                      }
                    }
      ...
      for (p = 0; p < max_active_parent; p++)
         if ((pp = &parents[p])->number >= 0 && pp->level == level &&
            ...
            for (o = 0; o < 8; o++) {
               ...
               k1 = (o/4)*z_block_half;
               double * array_from = bp1->array;
               double * array_to = bp->array;
...
#pragma omp task label(consolidate_block)                               \
  in(([num_vars*box_size]array_from)[start*box_size;number*box_size])   \
  out(([num_vars*box_size]array_to)[start*box_size;number*box_size])    \
  firstprivate(x_block_size, y_block_size, z_block_size,                \
               array_from, array_to, start, number)                     \
  ...
  default(none)
                 {
                   for (int v = start; v < start+number; v++) {
                     consolidate_block_task(x_block_size+2,
                                            y_block_size+2,
                                            z_block_size+2,
                                            &array_from[v*box_size],
                                            &array_to[v*box_size],
