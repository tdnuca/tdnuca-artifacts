...
void print_help_message(void)
{
  ...
  printf("=======================================\n\n");
  printf("--original_comm - (flag) uses the original halo communication routines\n");
  printf("                  (always original when `code != 0` or `stencil == 27`)\n");
  printf("--merge_checksum - (flag) does the checksum calculation in the same task\n");
  printf("                   as the stencil calculation\n");
}
...
void allocate(void)
{
  ...
  for (n = 0; n < max_num_blocks; n++) {
    blocks[n].number = -1;
    blocks[n].array = (double *) ma_aligned_malloc(num_vars*box_size,
                                            __FILE__, __LINE__);
    /* for (m = 0; m < num_vars; m++) { */
    /*   blocks[n].array[m] = (double *) */
    /*     ma_aligned_malloc(box_size, __FILE__, __LINE__); */
    /* } */
  }
